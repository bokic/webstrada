...
## CF 2025 `arguments` scope key order is Java HashMap bucket iteration

A UDF/component method that declares parameters and is called with **extra** named arguments (names matching no declared parameter) exposes those extra names in the `arguments` scope, but `StructKeyList(arguments)` iterates them in CF's internal Java `HashMap` bucket order, not declaration or insertion order. Verified on the RDS host: `component { function greet(who="world") { return structKeyList(arguments); } }` called `greet(zzz="x")` emits `zzz,WHO` (hashes: `zzz`→bucket 10, `WHO`→bucket 14), and `greet(aaa="1", zzz="2")` emits `aaa,EXTRA,zzz,WHO` — deterministic per instance but derived from String hash codes, the same class of CF-internal ordering as the colormodel/ImageGetMetadata cases above. The engine inserts declared params first (declaration order) then the extra named args, so enumeration order differs from CF for multi-param-plus-extra calls even though the values are byte-identical; it coincidentally matches CF when only one declared param is present (`X,zzz` vs CF's `X,zzz`). The cfm verify tests therefore assert argument *values* (`structKeyExists(arguments, ...)`, `arguments.name`) rather than `StructKeyList` order (see `tests/cfm/namedargs_comp_test.cfm` / `components/named_calc_extra.cfc`).
...
The Adobe CF 2025 server at `192.168.100.10` has `compileExtForInclude` set to a wildcard (CF Admin → Server Settings → "compile extensions for include"), so a `<cfinclude>` of a non-CFML file (`.txt`, `.sql`, `.xyz`, ...) is compiled and executed as CFML instead of being included as static content. Verified: including a file containing `raw #foo# <cfset z=7>` executes the `<cfset>` (a later `#z#` reads `7`) and strips the tag, whereas a CF install with an empty `compileExtForInclude` reads such files verbatim. This is also the stock CF 2025 default (the shipped `neo-runtime.xml` carries `<var name='compileextforinclude'><string>*</string></var>`), and it is what makes MangoBlog's setup work — `setupDatabase` does `<cfinclude template="mysql.sql">` and the include compiles the `<cfquery>` DDL/INSERT script that creates the `blog`/`entry`/... tables.

The engine reproduces the wildcard default: the `compileExtForInclude` server setting (webstrada-config.json, default `"*"`, matching CF's `IncludeTag.checkForType`) decides which extensions `<cfinclude>` compiles; an empty value restores the old engine behavior (only `.cfm`/`.cfml` compile, everything else is read and output raw). The `.txt` include test (`tests/cfm/cfinclude_static_test.cfm`) uses content that renders identically whether compiled or static; the compiled-vs-static distinction is covered by the `CfincludeTest.NonCfmlFileCompiledByDefault` / `CfincludeTest.StaticNonCfmlFileOutputRaw` unit tests, and the MangoBlog `.sql` pattern by `tests/cfm/cfinclude_sql_test.cfm`.
