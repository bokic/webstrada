# Research: GETTOTALSPACE cffunction implementation notes
...
> Implemented (2026-08-08): `cf_gettotalspace` in `src/cffunctions/`, a direct JIT call in
...
- Runtime stub: `cfml::cf_gettotalspace()` in `src/cf8.cpp:13673` throws `"Function GETTOTALSPACE is not implemented"`.
- Compiler: `GETTOTALSPACE` is in the zero-arg not-implemented function list (`src/compiler.cpp:1737`).
- No interpreter (`evalFunction`) dispatch entry.
- Symbol registered at `src/compiler.cpp:5398`.
...
| Docs/spec | ✅ Spec reference exists | `cfml_docs/CFML_FUNCTION_GETTOTALSPACE.md` |

## What GETTOTALSPACE does at the low C level

Return the total capacity (bytes) of the file system containing `path`.
...
```cpp
cfvariant *cf_gettotalspace(const cfvariant *a1);
```
...
Platform `statvfs` query. Unit tests + tracker updates.

## Ease of implementation

Easy. `statvfs()` call, mirrors `GetFreeSpace`.
