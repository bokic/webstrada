# Research: ISWDDX cffunction implementation notes
...
> Implemented (2026-08-08): `cf_iswddx` in `src/cffunctions/`, a direct JIT call in
...
- Runtime stub: `cfml::cf_iswddx()` in `src/cf8.cpp:14317` throws `"Function ISWDDX is not implemented"`.
- Compiler: `ISWDDX` is in the zero-arg not-implemented function list (`src/compiler.cpp:1741`).
- No interpreter (`evalFunction`) dispatch entry.
- Symbol registered at `src/compiler.cpp:5514`.
...
| Docs/spec | ✅ Spec reference exists | `cfml_docs/CFML_FUNCTION_ISWDDX.md` |

## What ISWDDX does at the low C level

Returns true if the value is a WDDX packet.
...
```cpp
cfvariant *cf_iswddx(const cfvariant *a1);
```
