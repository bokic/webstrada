# Research: ISTHREADINTERRUPTED cffunction implementation notes
...
> Implemented (2026-08-08): `cf_isthreadinterrupted` in `src/cffunctions/`, a direct JIT call in
...
- Runtime stub: `cfml::cf_isthreadinterrupted()` in `src/cf8.cpp:14293` throws `"Function ISTHREADINTERRUPTED is not implemented"`.
- Compiler: `ISTHREADINTERRUPTED` is in the zero-arg not-implemented function list (`src/compiler.cpp:1741`).
- No interpreter (`evalFunction`) dispatch entry.
- Symbol registered at `src/compiler.cpp:5508`.
...
| Docs/spec | ✅ Spec reference exists | `cfml_docs/CFML_FUNCTION_ISTHREADINTERRUPTED.md` |

## What ISTHREADINTERRUPTED does at the low C level

Returns true if the current thread was interrupted (cfthread interrupt).
...
```cpp
cfvariant *cf_isthreadinterrupted();
```
