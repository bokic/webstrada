...
cfvariant *cf_right(const cfvariant *str, const cfvariant *cnt) {
    if (!str || !cnt) throw webstrada::exception("Right requires exactly 2 arguments");
    ...
    if (count < 0) throw webstrada::exception("Right: count cannot be negative");
    auto *ret = new cfvariant(s.last(count));
    return ret;
}
