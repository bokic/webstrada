/**
 * @file fn_comparenocase.cpp
 * @brief CFML comparenocase() built-in.
 */
...
cfvariant *cf_comparenocase(const cfvariant *s1Val, const cfvariant *s2Val) {
    if (!s1Val || !s2Val) throw webstrada::exception("CompareNoCase requires exactly 2 arguments");
    webstrada::string s1 = const_cast<cfvariant*>(s1Val)->toString();
    webstrada::string s2 = const_cast<cfvariant*>(s2Val)->toString();
    int cmp = strcasecmp(s1.constData(), s2.constData());
