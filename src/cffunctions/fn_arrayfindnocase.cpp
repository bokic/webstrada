/**
 * @file fn_arrayfindnocase.cpp
 * @brief CFML arrayfindnocase() built-in.
 */
...
cfvariant *cf_arrayfindnocase(const cfvariant *arr, const cfvariant *val) {
    if (!arr || !val) throw webstrada::exception("ArrayFindNoCase: Missing argument(s)");
    if (arr->m_type != cfvariant::Array) {
        throw webstrada::exception("ArrayFindNoCase: First argument must be an array");
    }
    int foundIdx = 0;
