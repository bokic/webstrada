...
cfvariant *cf_minute(const cfvariant *arg) {
    if (!arg) throw webstrada::exception("Minute requires exactly 1 argument");
    double days = 0.0;
    if (arg->m_type == cfvariant::DateTime) {
        ...
        if (!parseDateTimeStr(const_cast<cfvariant*>(arg)->toString(), days)) {
            throw webstrada::exception("Minute: Invalid date/time value");
        }
    }
    struct tm tm = daysToTm(days);
    auto *ret = new cfvariant(tm.tm_min);
    return ret;
}
