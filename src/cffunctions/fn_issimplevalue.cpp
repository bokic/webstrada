/**
 * @file fn_issimplevalue.cpp
 * @brief CFML issimplevalue() built-in.
 */
...
cfvariant *cf_issimplevalue(const cfvariant *val) {
    ...
    ret->m_bool = val && (val->m_type != cfvariant::Array && val->m_type != cfvariant::Struct && val->m_type != cfvariant::Query && val->m_type != cfvariant::Xml && val->m_type != cfvariant::Component && val->m_type != cfvariant::Function);
    return ret;
}
