/**
 * @file fn_cjustify.cpp
 * @brief CFML cjustify() built-in.
 */
...
cfvariant *cf_cjustify(const cfvariant *str, const cfvariant *length) {
    if (!str || !length) throw webstrada::exception("CJustify requires exactly 2 arguments");
    ...
    int totalPadding = targetLen - currentLen;
    int rightPadding = totalPadding / 2;
    int leftPadding = totalPadding - rightPadding;
    webstrada::string res;
    for (int i = 0; i < leftPadding; ++i) res.append(" ");
    res.append(s);
    for (int i = 0; i < rightPadding; ++i) res.append(" ");
