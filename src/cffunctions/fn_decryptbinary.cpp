/**
 * @file fn_decryptbinary.cpp
 * @brief CFML decryptbinary() built-in.
 */
...
cfvariant *cf_decryptbinary(const cfvariant *binaryData, const cfvariant *key, const cfvariant *algorithm, const cfvariant *encoding, const cfvariant *IVorSalt, const cfvariant *iterations) {
    if (!binaryData || !key) throw webstrada::exception("DecryptBinary requires at least 2 arguments");
    if (iterations) {
        throw webstrada::exception("DecryptBinary: The iterations parameter is only supported with PBE algorithms, which are not available");
    }
    ...
    if (!encName.equals("UU") && !encName.equals("BASE64") && !encName.equals("HEX")) {
        throw webstrada::exception("DecryptBinary: Unknown encoding '" + encName + "'. Supported encodings: UU, Base64, Hex");
    }
