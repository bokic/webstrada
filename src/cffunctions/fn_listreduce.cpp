/**
 * @file fn_listreduce.cpp
 * @brief CFML listreduce() built-in.
 */
...
cfvariant *cf_listreduce(const cfvariant *list, const cfvariant *callback, const cfvariant *initialValue,
    ...
    if (!list || !callback) throw webstrada::exception("ListReduce requires a list and a callback");
    ...
    for (size_t i = 0; i < items.size(); i++) {
        std::vector<cfvariant> cbArgs = { acc, cfvariant(items[i]), cfvariant(static_cast<int>(i + 1)), listVal };
