...
cfvariant *cf_quarter(const cfvariant *date) {
    if (!date) throw webstrada::exception("Quarter requires exactly 1 argument");
    double days = getDaysOrThrow(date, "Quarter");
    struct tm tm = daysToTm(days);
    auto *ret = new cfvariant(tm.tm_mon / 3 + 1);
    return ret;
}
