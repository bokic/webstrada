/**
 * @file fn_isdateobject.cpp
 * @brief CFML isdateobject() built-in.
 */
...
cfvariant *cf_isdateobject(const cfvariant *value) {
    if (!value) throw webstrada::exception("IsDateObject requires exactly 1 argument");
    cfvariant res(cfvariant::Boolean);
    res.m_bool = (value->m_type == cfvariant::DateTime);
